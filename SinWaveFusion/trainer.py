...
from SinWaveFusion.models import EMA, NextNet
...
class Dataset(data.Dataset):
    def __init__(self, tensor, image_size, blurry_img, folder, exts=['jpg', 'jpeg', 'png', 'npy']):
        super().__init__()
        self.tensor = tensor
    ...
    def __getitem__(self, index):
        img = self.tensor
        crop_size = int(min(img.shape[-2:]) * 0.9)
        crop_size = crop_size if crop_size % 2 == 0 else crop_size - 1
        transform = transforms.Compose([
            transforms.RandomCrop(crop_size, pad_if_needed=False, padding_mode='constant')
        ])
        ...
        if self.blurry_img:
            ...
            img = self.tensor
            to_ll_tensor = (img).unsqueeze(0).to(device)
            ll, lh, hl, hh = dwt(to_ll_tensor)
            return transform(img), ll.squeeze(0)
        # else
...
class MultiscaleTrainer(object):
    ...
    def __init__(
        ...
        self.min_val_scale0 = None
        self.max_val_scale0 = None
        ...
        for i in range(n_scales):
            ...
            self.ds_list.append(Dataset(self.input[i], image_sizes[i], blurry_img, self.input_paths[i]))
            self.dl_list.append(
                cycle(data.DataLoader(self.ds_list[i], batch_size=train_batch_size, shuffle=True)))
            ...
            if i > 0:
                ...
                Data = next(self.dl_list[i])
                norm, min_val, max_val = img_normalize(Data)
                self.min_val_scale0 = min_val
                self.max_val_scale0 = max_val
                self.data_list.append((Data.to(self.device)))
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            for i in range(self.gradient_accumulate_every):
                ...
                total_latency.append(step_elapsed_time)
                ...
                if self.step < 12:
                    ...
                    input_size = (1, 12, h//2, w//2)
                    tt = torch.randint(0, 100, (1,), device=self.device).long()
                    ss = s.to(self.device)
                    
                    summary_info = summary(model_test, input_size=input_size, t=tt, scale=ss, device=self.device)
                    # Total_mult-adds 출력
                    mult_adds = summary_info.total_mult_adds
                    params = summary_info.total_params
                    memory = summary_info.total_output_bytes / (1024 * 1024)
                    
                    gflops = mult_adds / 1e9

                    self.scale_results[current_scale] = {
                        'mult_adds': mult_adds,
                        'params': params,
                        'memory': memory,
                        'gflops': gflops
                    ...
                    print(f"Step {self.step}, Scale {current_scale} - GFLOPS: {gflops:.4f}, Parameters: {params/1e6:.2f}M, Memory: {memory:.2f}MB")

                if self.step == 10:
                    # 모든 scale의 총 GFLOPS 및 평균 계산
                    total_gflops = sum(result['gflops'] for result in self.scale_results.values())
                    total_params = sum(result['params'] for result in self.scale_results.values())
                    total_memory = sum(result['memory'] for result in self.scale_results.values())
                    
                    # 평균 GFLOPS 계산
                    avg_gflops = total_gflops / len(self.scale_results)
                    
                    # 결과 출력
                    print("\n=== 결과 요약 ===")
                    print(f"총 GFLOPS: {total_gflops:.4f}")
                    print(f"평균 GFLOPS: {avg_gflops:.4f}")
                    print(f"총 Parameters: {total_params/1e6:.2f}M")
                    print(f"총 Memory: {total_memory:.2f}MB")
                    
                    # 결과 저장
                    self.MPARAMS = total_params / 1e6
                    self.GFLOPS = total_gflops
                    self.AVG_GFLOPS = avg_gflops
                    self.MEMORYS = total_memory
                else:
                    pass
            ...
            if self.step % self.save_and_sample_every == 0:
                ...
                img = iwt(ll, lh, hl, hh)
                all_images = img_denormalize(img, self.min_val_scale0, self.max_val_scale0)
                utils.save_image(all_images, str(self.results_folder / f'sample-{milestone}.png'), nrow=4)
                self.save(milestone)
        
        name = str(self.results_folder).split("/")[-1]
        average_latency = sum(total_latency) / len(total_latency)
        with open(f"combined_results_{name}.txt", "w") as combined_file:
            combined_file.write(f"Total GFLOPs: {GFLOPS:.2f}\n")
            combined_file.write(f"Average Latency: {average_latency:.4f}seconds\n")
            combined_file.write(f"Param Count in Millions: {MPARAMS:.2f}M\n")
            combined_file.write(f"Total Memory Size of the output tensor: {MEMORYS:.2f}MB")
        print('결과가 combined_results.txt 파일로 저장되었습니다.')
    ...
    def sample_scales(self, scale_mul=None, batch_size=16, custom_sample=False, custom_image_size_idxs=None,
        ...
        for i in range(n_scales):
            ...
            if start_noise and i == 0:
                scale_0 = self.ema_model.sample(batch_size=batch_size, scale_0_size=scale_0_size, s=custom_scales[i])
                ll = torch.split(scale_0, split_size_or_sections=3, dim=1)[0]
                lh = torch.split(scale_0, split_size_or_sections=3, dim=1)[1]
                hl = torch.split(scale_0, split_size_or_sections=3, dim=1)[2]
                hh = torch.split(scale_0, split_size_or_sections=3, dim=1)[3]
                scale_0 = iwt(ll, lh, hl, hh)
                scale_0 = img_denormalize(scale_0, self.min_val_scale0, self.max_val_scale0)
                samples_from_scales.append(scale_0)
                ...
                scale_1 = self.ema_model.sample_via_scale(batch_size,
                                                    ...
                                                    mode=None)
                ll = torch.split(scale_1, split_size_or_sections=3, dim=1)[0]
                lh = torch.split(scale_1, split_size_or_sections=3, dim=1)[1]
                hl = torch.split(scale_1, split_size_or_sections=3, dim=1)[2]
                hh = torch.split(scale_1, split_size_or_sections=3, dim=1)[3]
                scale_1 = iwt(ll, lh, hl, hh)
                scale_1.clamp_(0., 1.)
                samples_from_scales.append(scale_1)

            final_img = samples_from_scales[i]
    ...
    def image2image(self, input_folder='', input_file='', mask='', 
                    hist_ref_path='', image_name='', start_s=1, custom_t=None, 
                    batch_size=16, scale_mul=(1, 1), device=None, 
                    use_hist=False, save_unbatched=True, auto_scale=None, mode=None):
        if custom_t is None:
        ...
        if auto_scale is not None:
            ...
            input_img = input_img.resize((after_width, after_height), Image.LANCZOS)
            image_size = input_img.size
            if scaler > 1:
                image_size = (int(image_size[0] / scaler), int(image_size[1] / scaler))
                new_width = image_size[0] if image_size[0] % 2 == 0 else image_size[0] + 1
                new_height = image_size[1] if image_size[1] % 2 == 0 else image_size[1] + 1
        ...
        for i in range(self.n_scales-start_s):
            ...
            final_img = (samples_from_scales[i])
            iwt = IDWT_2D("haar")
            final_ll = torch.split(final_img, split_size_or_sections=3, dim=1)[0]
            final_lh = torch.split(final_img, split_size_or_sections=3, dim=1)[1]
            final_hl = torch.split(final_img, split_size_or_sections=3, dim=1)[2]
            final_hh = torch.split(final_img, split_size_or_sections=3, dim=1)[3]
            final_img = iwt(final_ll, final_lh, final_hl, final_hh)
            final_img.clamp_(0.0, 1.0)
