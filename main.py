...
import os
import torchvision
from SinWaveFusion.functions import create_img_scales
from SinWaveFusion.models import NextNet, MultiScaleGaussianDiffusion
from SinWaveFusion.trainer import MultiscaleTrainer
...
def main():
    ...
    parser.add_argument("--start_t_style", help='starting T at last scale for style transfer', default=5, type=int)
    ...
    # Dataset
    parser.add_argument("--dataset_folder", help='choose dataset folder.', default='.\datasets\seascape')
    ...
    parser.add_argument("--train_num_steps", help='total training steps.', default=80001, type=int)
    ...
    parser.add_argument("--train_lr", help='starting lr.', default=2e-3, type=float)

    parser.add_argument("--start_t_resolution", help='starting T at last scale for SR', default=40, type=int)
 
    def get_milestones(train_num_steps):
        if train_num_steps == 90001:
            return [15, 30, 52, 60, 67, 82]
        elif train_num_steps == 120001:
            return [20, 40, 70, 80, 90, 110]
        else:
            base_steps = 120001
            scale_factor = train_num_steps / base_steps
            base_milestones = [20, 40, 70, 80, 90, 110]
            return [int(m * scale_factor) for m in base_milestones]
    
    args, _ = parser.parse_known_args()
    default_milestones = get_milestones(args.train_num_steps)
    parser.add_argument("--sched_k_milestones", nargs="+", help='lr scheduler steps x 1000.',
                        default=default_milestones, type=int)
    ...
    #sched_milestones = [val * 100 for val in args.sched_k_milestones]
    ...
    sizes, rescale_losses, scale_factor, n_scales, tensor_list = create_img_scales(args.dataset_folder, args.image_name,
    ...
    ScaleTrainer = MultiscaleTrainer(
            ...
            ema_decay=0.995,  # exponential moving average decay # 0.999
            fp16=False,
    ...
    if args.mode == 'train':
        ...
        clip_cfg = {"clip_model_name": "ViT-B/32", #ViT-B/16
        ...
        strength = 0.2
        quantile = 0.0 # allow to change the whole image #0.0
        llambda = 0.05
        stop_guidance = 6 
        #stop_guidance = args.clip_style_stop_guidance
          # at the last scale, disable the guidance in the last x steps in order to avoid artifacts from CLIP
    ...
    elif args.mode == 'style_transfer' or args.mode == 'harmonization' or args.mode == 'resolution':
        ...
        if args.mode == 'style_transfer':
            ...
            start_s = n_scales - 1
            start_t = args.start_t_resolution
            use_hist = True
        else:
            # start diffusion from last scale
            start_s = n_scales - 1
        ...
        ScaleTrainer.image2image(input_folder=i2i_folder, input_file=args.input_image, 
                                 mask=args.harm_mask, 
